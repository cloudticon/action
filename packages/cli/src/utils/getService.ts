...
class Service {
  ...
  logs() {
    this.stopProcess();
    this.process = kubectlStream(['logs', '-f', `deploy/${this.name}`, '-n', this.namespace]);
  ...
  shell() {
    this.stopProcess();
    kubectlStream(['exec', '-it', `deploy/${this.name}`, '-n', this.namespace, '--', 'ash'], {
      cwd: process.cwd(),
      detached: true,
      stdio: 'inherit',
    });
  ...
  async getPodName() {
    const { items } = await kubectlJson(['get', 'pod', '-n', this.namespace, '-o', 'json']);
  ...
  async devMode(outDir: string) {
    if (!this.isDevMode) {
      ...
      await patchDeploy(this.name, this.namespace, [
        ...
        // { op: 'add', path: '/metadata/annotations/ct-snapshot', value: JSON.stringify(this.deploy) },
        {
          op: 'replace',
          path: '/spec/template/spec/containers/0/command',
          value: ['npx', 'nodemon', '--watch', `/app/${outDir}/**`, '--ext', 'js', '--exec', 'yarn start'],
        },
        {
          op: 'replace',
  ...
  startDevProcess() {
    ...
    this.devProcess = kubectlStream(['exec', '-i', `deploy/${this.name}`, '-n', this.namespace, '--', 'ash']);
  ...
  async devModeOff() {
    if (this.isDevMode) {
      ...
      await patchDeploy(this.name, this.namespace, [
        ...
        //   value: { httpGet: { path: '/health', port: 8080 } },
        // },
      ]);
      await waitForDeployAvailable(this.name, this.namespace);
      await this.refresh();
...
export const patchDeploy = async (name: string, namespace: string, data: any) => {
  await kubectl(['patch', 'deployment', name, '-n', namespace, '--type', 'json', '-p', JSON.stringify(data)]);
};

export const getDeploy = async (name: string, namespace: string) => {
  const deploy = await kubectlJson(['get', 'deploy', name, '-o', 'json', '-n', namespace]);
  deploy.isReady = deploy.status.readyReplicas === deploy.status.replicas;
  return deploy;
};

export const waitForDeployAvailable = async (name: string, namespace: string) => {
